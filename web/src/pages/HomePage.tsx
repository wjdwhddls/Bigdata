...
export default function HomePage({ categoryFilter = null, onCategoryChange, currentUserId, newUserAgeGroup }: HomePageProps) {
  ...
  // 자동문 오버레이 JSX (두 return문 모두에서 사용)
  const doorOverlay = doorPhase !== 'idle' && doorPhase !== 'done' && (
          ...
          <div className="relative z-10 text-center select-none px-4 flex flex-col items-center">
            ...
            >
              <button
                onClick={() => {
                  setDoorPhase('closed')
                  // 닫힌 문이 렌더/paint된 후 showProducts 전환
                  setTimeout(() => {
                    setShowProducts(true)
                  }, 300)
                  // showProducts 전환 후 문 열기 시작 (rAF으로 paint 보장)
                  setTimeout(() => {
